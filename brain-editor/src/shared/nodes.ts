...
let global_bus_dictionaries: { [key: string]: any } = {};

const gpt_endpoint = '/v1/chat/completions';
const gpt_url = 'https://api.openai.com';
const default_gpt_model = 'gpt-3.5-turbo';

async function call_gpt(messages: any[], api_key: string, url: string = gpt_url, model: string = default_gpt_model, grammar: string | undefined = undefined): Promise<string> {
    const headers = {
    ...
    const final_url = url + gpt_endpoint;
...
async function query_wikipedia(query: string, milvus_url: string, top_k: number = 3): Promise<any> {
    const headers = {
        'Content-Type': 'application/json'
    };
...
async function query_milvus(class_key: string, query: string, milvus_url: string, top_k: number = 3): Promise<any> {
    const headers = {
        'Content-Type': 'application/json'
    };
...
async function create_milvus_collection(class_key: string, milvus_url: string): Promise<any> {
...
async function insert_milvus(class_key: string, text: string, milvus_url: string): Promise<any> {
    const headers = {
        'Content-Type': 'application/json'
    };
...
async function delete_milvus_collection(class_key: string, milvus_url: string): Promise<any> {
    if (class_key === "" || class_key === "wiki") {
        return;
    }
...
class EventEmitter {
    private events: { [event: string]: Array<{ listener: Function, sender: any }> };

    constructor() {
        this.events = {};
    }

    on(event: string, listener: Function, sender: any): void {
...
function containsWords(words: string[], text: string): boolean {
    function escapeRegExp(string: string): string {
        return string.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    }

    const sanitizedWords = words.map(escapeRegExp);
    const regex = new RegExp('\\b(' + sanitizedWords.join('|') + ')\\b', 'i');
...
function startPolling(requestId: string): void {
    ...
    fetch(`http://192.168.0.7:2702/get-audio/${requestId}`)
        ...
        .finally(() => {
            isPolling = false;
            setTimeout(() => startPolling(requestId), 1000);
        });
}

function startAudioGeneration(text: string): void {
...
async function saveTotalAudio(): Promise<void> {
    let combinedAudioData: Uint8Array[] = [];
    let headerLength = 44;
...
function concatenateUint8Arrays(arrayList: Uint8Array[]): Uint8Array {
...
function updateWavHeader(buffer: Uint8Array): void {
...
async function create_simple_vector_db_collection(collection_name: string, url: string): Promise<void> {
    console.log("creating collection: " + collection_name);
    await fetch(url + "/create_collection", {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function insert_simple_vector_db(collection_name: string, title: string, text: string, url: string): Promise<void> {
    console.log("inserting into simple vector db: " + text);
    await fetch(url + "/add_document", {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function delete_simple_vector_db_collection(collection_name: string, url: string): Promise<void> {
    await fetch(url + "/delete_collection", {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function delete_simple_vector_db_document(collection_name: string, document_id: string, url: string): Promise<void> {
    await fetch(url + "/delete_document", {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function get_document_by_id(collection_name: string, document_id: string, url: string): Promise<any> {
    const insert_response = await fetch(url + "/get_document_by_id", {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function get_document_by_title(collection_name: string, title: string, url: string): Promise<any> {
    const insert_response = await fetch(url + "/get_document_by_title", {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function get_similar_documents_by_cos(collection_name: string, document_id: string, top_k: number, url: string): Promise<any> {
    const insert_response = await fetch(url + "/get_similar_documents_by_cos", {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function get_similar_documents_by_euclidean(collection_name: string, query: string, top_n: number, url: string): Promise<any> {
    const query_response = await fetch(url + "/get_similar_documents_by_euclidean", {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
    ...
    const response_json = await query_response.json();
    return response_json;
}

async function collection_exists(collection_name: string, url: string): Promise<boolean> {
    console.log("checking if collection exists " + collection_name);
    const exists_response = await fetch(url + "/collection_exists", {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
    ...
    const response_json = await exists_response.json();
    return response_json;
}
